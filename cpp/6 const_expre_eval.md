...
| 特性 | `const` promise | `constexpr` expectation  | `consteval` requirement |
| --- | --- | --- | --- |
...
ex 
`constexpr std::array<int, 3> prime_numbers = {2, 3, 5};`
...
```cpp
consteval int sum_to_n(int n) {
    std::vector<int> v; // Dynamic allocation at compile-time!
    for(int i = 1; i <= n; ++i) {
        v.push_back(i);
    }
    return std::accumulate(v.begin(), v.end(), 0); 
    // Vector is destroyed here. Memory is freed. This is "Transient".
}


int main() {
    constexpr int total = sum_to_n(10); // Works: 'total' is just an int.
    static_assert(total == 55);

    int input;
    std::cin >> input;
    sum_to_n(input); // compile error: not possible in compile time
}
